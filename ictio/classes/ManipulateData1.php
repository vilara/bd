 ...
 //método que cria lista das Excursões
   ...
   public function geraListaExc1($aparelho,$flag1,$sec){
	   ...
	   //  echo "<option value=\""."index.php?secoes=consultas/registros"."\">".$prod["1"]."</option>\n";
           ...
           //método que cria lista das Excursões
   public function geraListaExc3($aparelho,$flag1,$sec){
        $this->sql = "SELECT * FROM tb_excursao ORDER BY ex_numero ASC";
        $this->qr = self::execSQL($this->sql);
		while($prod = self::results())
	    {
  ...
  //método que mostra o nome do codigo a partir do id
           public function geraListaCodMalhRed($aparelho,$excc){
	        ...
	        echo "<option value=\"".$prod["0"]."\">".$prod["2"]."</option>\n";
             ...
             //método que mostra o nome do codigo a partir do id
           public function geraListaCorrCodMalhRed($aparelho){
    		$this->sql = "SELECT * FROM tb_limno_corr WHERE lim_corr_aparelho = '$aparelho' ORDER BY id_tb_limno ASC";
            $this->qr = self::execSQL($this->sql);
		    while($prod = self::results())
	        {
	        echo "<option value=\"".$prod["0"]."\">".$prod["2"]."</option>\n";
            ...
            //método que mostra o nome do codigo a partir do id
           public function geraListaCodMalhRed1($aparelho){
    		$this->sql = "SELECT * FROM tb_limno WHERE lim_aparelho = '$aparelho' ORDER BY lim_cod ASC";
            $this->qr = self::execSQL($this->sql);
		    while($prod = self::results())
	        {
	        echo "<option value=\"".$prod["0"]."\">".$prod["2"]."</option>\n";
         ...
         /*  public function geraListaCodArr($excc){
            ...
            public function geraListaCodArr1(){
    		$this->sql = "SELECT * FROM tb_limno_arr WHERE id_tb_limno_arr > 0 ORDER BY lim_arr_cod ASC";
            $this->qr = self::execSQL($this->sql);
		    while($prod = self::results())
	        {
	        echo "<option value=\"".$prod["0"]."\">".$prod["1"]."</option>\n";
           ...
           public function geraListaCodOutr($excc,$aparelho){
	        ...
	        echo "<option value=\"".$prod["0"]."\">".$prod["5"]."</option>\n";
             ...
             //método que mostra o nome da espécie a partir do código
           public function geraListaCodOutr1($aparelho){
    		$this->sql = "SELECT * FROM tb_limno_outr WHERE tb_limno_outr.id_tb_limno_outr > 0 AND tb_limno_outr.lim_outr_aparelho = '$aparelho' ORDER BY lim_outr_cod ASC";
            $this->qr = self::execSQL($this->sql);
		    while($prod = self::results())
	        {
	        echo "<option value=\"".$prod["0"]."\">".$prod["5"]."</option>\n";
              ...
              //método que mostra o nome da espécie a partir do código
           public function geraListaCodIg($excc,$aparelho){
    		$this->sql = "SELECT * FROM tb_limno_ig,tb_local_ig WHERE tb_limno_ig.lim_ig_local = tb_local_ig.id_tab_local AND tb_limno_ig.id_tb_limno > 0 AND tb_limno_ig.lim_ig_aparelho = '$aparelho' $excc ORDER BY tb_local_ig.id_tab_local ASC";
            $this->qr = self::execSQL($this->sql);
		    while($prod = self::results())
	        {
	        echo "<option value=\"".$prod["0"]."\">".$prod["4"]."</option>\n";
               ...
               //método que mostra o nome da espécie a partir do código
           public function geraListaCodCorr($aparelho){
    		$this->sql = "SELECT * FROM tb_limno_corr WHERE tb_limno_corr.id_tb_limno > 0 AND tb_limno_corr.lim_corr_aparelho = '$aparelho' ORDER BY tb_limno_corr.id_tb_limno ASC";
            $this->qr = self::execSQL($this->sql);
		    while($prod = self::results())
	        {
	        echo "<option value=\"".$prod["0"]."\">".$prod["2"]."</option>\n";
 ...
 //método que mostra o nome da espécie a partir do código
           ...
           public function NrEspecie($cod){
            ...
            return $this->data["0"];
              ...
              public function EspCatrof(){
           	$this->sql = "SELECT * FROM tb_especies WHERE id_tab_especie > 0 GROUP BY esp_catrof ORDER BY esp_catrof ASC";
            $this->qr = self::execSQL($this->sql);
		    while($prod = self::results())
	        {
            ...
            //método que mostra o nome da espécie a partir do código
           public function NomeLocal($cod){
    		$this->sql = "SELECT * FROM pesc_especie WHERE id_esp = '$cod'";
            $this->qr = self::execSql($this->sql);
            $this->data = self::listQr($this->qr);
            return $this->data["1"];
    	   	}
             ...
             //método que mostra o nome da espécie a partir do código
           public function NomeLoc($cod){
    		$this->sql = "SELECT * FROM tb_local WHERE id_tab_local = '$cod'";
            $this->qr = self::execSql($this->sql);
            $this->data = self::listQr($this->qr);
            return $this->data["1"];
    	   	}
   ...
   public function geraListaEspecie(){
        $this->sql = "SELECT * FROM tb_especies WHERE id_tab_especie != 513 OR id_tab_especie != 825 GROUP BY esp_especie ORDER BY esp_especie ASC";
        $this->qr = self::execSQL($this->sql);
		while($prod = self::results())
	    {
        ...
        //método que cria lista das Excursões
   public function geraListaLocc(){
        $this->sql = "SELECT * FROM tb_local GROUP BY loc_descricao ORDER BY loc_ordem ASC";
        $this->qr = self::execSQL($this->sql);
		while($prod = self::results())
	    {
	    echo "<option value=\"".$prod["0"]."\">".$prod["1"]."</option>\n";
            ...
            //método que mostra o nome da local da pesca
           public function NomeLocalPesc($loc){
    		$this->sql = "SELECT * FROM pesc_localidade WHERE id_locali = '$loc'";
            $this->qr = self::execSql($this->sql);
            $this->data = self::listQr($this->qr);
            return $this->data["1"];
    	   	}
