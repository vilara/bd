...
//método que cria lista dos códigos de campo de Malhadeira e Redinha
   ...
   public function geraListaEspecie(){
	    ...
	    echo "<option value=\"".$prod["3"]."\">".$prod["3"]."</option>\n";
            ...
            //método que cria lista das Excursões
   public function geraListaEspecieMapa(){
        $this->sql = "SELECT * FROM tb_especies GROUP BY esp_especie ORDER BY esp_especie ASC";
        $this->qr = self::execSQL($this->sql);
		while($prod = self::results())
	    {
   ...
   public function geraListaCodCampoArr($exc){
	    ...
	    echo "<option value=\"".$prod["0"]."\">".$prod["1"]."</option>\n";
                 ...
                 //método que cria lista dos códigos de campo de Arrasto de fundo
   public function geraListaCodCampoCorr($exc){
        $this->sql = "SELECT * FROM tb_limno_corr WHERE lim_corr_excursao = '$exc' ORDER BY id_tb_limno ASC";
        $this->qr = self::execSQL($this->sql);
		while($prod = self::results())
	    {
	    echo "<option value=\"".$prod["0"]."\">".$prod["2"]."</option>\n";
   ...
   public function geraListaCodCampoOutr($exc){
	    ...
	    echo "<option value=\"".$prod["0"]."\">".$prod["5"]."</option>\n";
         ...
         //método que cria lista dos códigos de campo de Outros Aparelhos
   public function geraListaCodCampoOutr1($exc){
        $this->sql = "SELECT * FROM tb_limno_outr WHERE lim_outr_aparelho != 'Puça' AND lim_outr_aparelho != 'Tarrafa' AND lim_outr_aparelho != 'Espinhel' AND  lim_outr_excursao = '$exc'  ORDER BY id_tb_limno_outr ASC";
        $this->qr = self::execSQL($this->sql);
		while($prod = self::results())
	    {
	    echo "<option value=\"".$prod["0"]."\">".$prod["5"]."</option>\n";
         ...
         //método que cria lista dos códigos de campo de Outros Aparelhos
   public function geraListaCodCampoIg($exc){
        $this->sql = "SELECT * FROM tb_limno_ig WHERE lim_ig_excursao = '$exc'  ORDER BY id_tb_limno ASC";
        $this->qr = self::execSQL($this->sql);
		while($prod = self::results())
	    {
	    echo "<option value=\"".$prod["0"]."\">".$prod["4"]."</option>\n";
         ...
         //método que cria lista dos códigos de campo de Outros Aparelhos
   public function geraListaCodCampoOutrTarr($exc){
        $this->sql = "SELECT * FROM tb_limno_outr WHERE lim_outr_aparelho = 'Tarrafa' AND  lim_outr_excursao = '$exc'  ORDER BY id_tb_limno_outr ASC";
        $this->qr = self::execSQL($this->sql);
		while($prod = self::results())
	    {
	    echo "<option value=\"".$prod["0"]."\">".$prod["5"]."</option>\n";
