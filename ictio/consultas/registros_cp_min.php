...
if (url != " no") window.location = url;
 ...
 <?php
  ...
  //  echo "$exc-$flag-$aparelho-$flag1";
?>
<div id="conteudo">
   <h3>Parâmetros da consulta de espécies capturadas</h3>

   <form name="navegador" id="formulario" method="post" action="index.php?secoes=consultas/registros_cp_min&flag=1&flag1=1&flag2=1&aparelho=<?php echo"$aparelho"; ?>&exc=<?php echo"$exc"; ?>" enctype="multipart/form-data">
   <fieldset>
    ...
    <tr>
       ...
       <SELECT name='exc' id="exc" type='text'  tabindex="1"  onchange="destino1()" >
           ...
           ?>
       <option value='index.php?secoes=consultas/registros_cp_min&flag=0'>&nbsp;</option>
       <option value="index.php?secoes=consultas/registros_cp_min&exc=Geral&flag=1&aparelho=<?php echo "$aparelho"; ?>&flag1=<?php echo "$flag1"; ?>">Geral</option>

               <option value="index.php?secoes=consultas/registros_cp_min&exc=Todas&flag=1&aparelho=<?php echo "$aparelho"; ?>&flag1=<?php echo "$flag1"; ?>">Todas</option>

           <?php
               $lista = new ManipulateData1();
               $lista->geraListaExc1($aparelho,$flag1,registros_cp_min);
           ?>
       </select></label></td>
       <td>&nbsp;</td>
       <td>&nbsp;</td>
    ...
    <tr>
      ...
      <td><label for="inicial">
           ...
           <SELECT NAME='aparelho' onchange="destino()" type='text' id='lim_aparelho' tabindex="4">
             <option value='index.php?secoes=consultas/registros_cp_min&exc=<?php echo"$exc";?>&flag=1&flag1=0'>Selecione</option>
             ...
             <option value='index.php?secoes=consultas/registros_cp_min&aparelho=Redinha&exc=<?php echo"$exc";?>&flag=1&flag1=1' <?php echo"$re";?>>Redinha</option>  >-->
             <option value='index.php?secoes=consultas/registros_cp_min&aparelho=Malhadeira&exc=<?php echo"$exc";?>&flag=1&flag1=1' <?php echo"$ma";?>>Malhadeira</option>
<!--               <option value='index.php?secoes=consultas/registros_cp_min&aparelho=Arrasto de fundo&exc=<?php echo"$exc";?>&flag=1&flag1=1' <?php echo"$ar";?>>Arrasto Bentônico</option>
             <option value='index.php?secoes=consultas/registros_cp_min&aparelho=Puça&exc=<?php echo"$exc";?>&flag=1&flag1=1' <?php echo"$pu";?>>Puça</option>
              <option value='index.php?secoes=consultas/registros_cp_min&aparelho=Tarrafa&exc=<?php echo"$exc";?>&flag=1&flag1=1' <?php echo"$ta";?>>Tarrafa</option>
             <option value='index.php?secoes=consultas/registros_cp_min&aparelho=Espinhel&exc=<?php echo"$exc";?>&flag=1&flag1=1' <?php echo"$espii";?>>Espinhel</option>
    ...
    if ($flag1 == 1)
    {
        ...
        ?>
            <tr>
                ...
                <td><label for="inicial">
                <?php
                if($exc != "Todas" AND $exc != "Geral")
                {
                $excc = " "."AND tb_limno.lim_excursao = $exc";
                }
               else if($exc == "Geral")
                {$excc = " "."AND tb_limno.lim_excursao = '0'";}

               else if ($exc == "Todas")  {$excc = " ";}
               //  echo "$excc";
                ?>
        ...
        ?>
            ...
            {$excc = " "."AND tb_limno_arr.lim_arr_excursao = $exc";}
                ...
                {$excc = " "."AND tb_limno.lim_arr_excursao = '0'";}
        ...
        if ($aparelho == "Espinhel" OR $aparelho == "Puça" OR $aparelho == "Espinhel-ant" OR $aparelho == "Tarrafa")
        {
        ?>
            <tr>
            <td>3) Código Limno:</td>
            ...
            if($exc == "Geral")
                {$excc = "   "." AND tb_limno_outr.lim_outr_excursao = 0";}; /* echo "$exc";*/
            if ($aparelho == 'Espinhel-ant'){$aparelho = "Espinhel";$ee = 'a';}
           ?>
            <SELECT name='cod' type='text' tabindex="1">
            <OPTION value="Todos" >Todos</option>
            <?php
...
?>
    ...
    {
    include_once "registros_mostrar_cp_min.php";
    }
    ?>