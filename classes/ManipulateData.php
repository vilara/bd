...
//método que cria lista das Espécies Cadastradas
   ...
   public function geraListaOrdem(){
        ...
        //método que cria lista das Espécies Cadastradas
   public function geraListaFamilia(){
	    ...
	    echo "<option value=\"".$prod["2"]."\">".$prod["2"]."</option>\n";
         ...
         //método que cria lista das Espécies Cadastradas
   public function geraListaLocais(){
        $this->sql = "SELECT * FROM tb_local WHERE loc_aparelho = 'Malhadeira' OR loc_aparelho = 'Malhadeira_exp' OR loc_aparelho = 'Arrasto' GROUP BY loc_descricao ORDER BY loc_ordem ASC";
        $this->qr = self::execSQL($this->sql);
		while($prod = self::results())
	    {
	    echo "<option value=\"".$prod["0"]."\">".$prod["1"]."</option>\n";
