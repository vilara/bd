...
//método que cria lista dos códigos de campo de Malhadeira e Redinha
   ...
   public function geraListaPesc(){
	    ...
	    echo "<option value=\"".$prod["0"]."\">".$prod["1"]."</option>\n";
         ...
         //método que cria lista dos códigos de campo de Arrasto de fundo
   public function geraListaPescF(){
        $this->sql = "SELECT * FROM pesc_pescador WHERE ori = 'F' OR ori = 'A' ORDER BY nome ASC";
        $this->qr = self::execSQL($this->sql);
		while($prod = self::results())
	    {
	    echo "<option value=\"".$prod["0"]."\">".$prod["1"]."</option>\n";
         ...
         //método que cria lista dos códigos de campo de Arrasto de fundo
   public function geraListaPescC(){
        $this->sql = "SELECT * FROM pesc_pescador WHERE ori = 'C' ORDER BY nome ASC";
        $this->qr = self::execSQL($this->sql);
		while($prod = self::results())
	    {
	    echo "<option value=\"".$prod["0"]."\">".$prod["1"]."</option>\n";
 ...
 //método que cria lista das Excursões
   public function geraListaCateg(){
        $this->sql = "SELECT * FROM pesc_especie ORDER BY cat ASC";
        $this->qr = self::execSQL($this->sql);
		while($prod = self::results())
	    {
	    echo "<option value=\"".$prod["0"]."\">".$prod["1"]."</option>\n";
   ...
   public function geraListaPesq(){
	    ...
	    echo "<option value=\"".$prod["2"]."\">".$prod["2"]."</option>\n";
           ...
           public function NomeLocal($cod){
    		$this->sql = "SELECT * FROM pesc_localidade WHERE id_locali = '$cod'";
            $this->qr = self::execSql($this->sql);
            $this->data = self::listQr($this->qr);
            return $this->data["1"];
    	   	}
             ...
             //método que mostra o nome da espécie a partir do código
           public function NomePesc($cod){
    		$this->sql = "SELECT * FROM pesc_pescador WHERE id_pescador = '$cod'";
            $this->qr = self::execSql($this->sql);
            $this->data = self::listQr($this->qr);
            return $this->data["1"];
    	   	}
