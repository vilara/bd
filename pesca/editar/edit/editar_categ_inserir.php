...
<label for="especie">
        ...
        14) Coleção:  </label>
            ...
            <?php
          //   include "config.php";
            $res = mysql_query("SELECT * FROM tb_malhadeira_biotico WHERE malh_voucher LIKE '%UNIR%' ORDER BY malh_voucher ASC");
            while($registro = mysql_fetch_row($res))
            {
            $malh_voucher = $registro[20];
            echo "<option value=\"$malh_voucher\">$malh_voucher</option>\n";
            }
            ?></select>
...
<form id="forms" method="post" action="index.php?secoes=edit/editar_malh_enviar&idd=<?php echo"$id";?>&aparelho=Redinha&cod=<?php echo"$cod"; ?>" enctype="multipart/form-data">
        ...
        4) Coleção:  </label>
            ...
            <?php
            include "config.php";
            $res = mysql_query("SELECT * FROM tb_redinha_biotico WHERE red_voucher LIKE '%UNIR%' ORDER BY red_voucher ASC");
            while($registro = mysql_fetch_row($res))
            {
            $red_voucher = $registro[10];
            echo "<option value=\"$red_voucher\">$red_voucher</option>\n";
            }
            ?></select>
           <label for="aparelho">
        5) Tecido:  </label>
...
<label for="comprimento">
 ...
 <label for="gr">
        ...
        18) Coleção:  </label>
            ...
            <?php
            // include "config.php";
            $res = mysql_query("SELECT * FROM tb_espinhel_biotico WHERE espi_voucher LIKE '%UNIR%' ORDER BY espi_voucher ASC");
            while($registro = mysql_fetch_row($res))
            {
            $espi_voucher = $registro[20];
            echo "<option value=\"$espi_voucher\">$espi_voucher</option>\n";
            }
            ?></select>
...
<form id="forms" method="post" action="index.php?secoes=edit/editar_malh_enviar&idd=<?php echo"$id";?>&aparelho=Arrasto de fundo&cod=<?php echo"$cod"; ?>" enctype="multipart/form-data">
        ...
        5) Coleção: </label>
            ...
            <?php
            include "config.php";
            $res = mysql_query("SELECT * FROM tb_arrastofundo_biotico WHERE arr_voucher LIKE '%UNIR%' ORDER BY arr_voucher ASC");
            while($registro = mysql_fetch_row($res))
            {
            $arr_voucher = $registro[10];
            echo "<option value=\"$arr_voucher\">$arr_voucher</option>\n";
            }
            ?></select>
...
<form id="forms" method="post" action="index.php?secoes=edit/editar_malh_enviar&idd=<?php echo"$id";?>&aparelho=Outros&cod=<?php echo"$cod"; ?>" enctype="multipart/form-data">
        ...
        5) Coleção: </label>
            ...
            <?php
            include "config.php";
            $res = mysql_query("SELECT * FROM tb_outros_biotico WHERE outr_voucher LIKE '%UNIR%' ORDER BY outr_voucher ASC");
            while($registro = mysql_fetch_row($res))
            {
            $outr_voucher = $registro[10];
            echo "<option value=\"$outr_voucher\">$outr_voucher</option>\n";
            }
            ?></select>
